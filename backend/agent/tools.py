...
def search_legal_clauses(
    query: str, target_date: str | None = None, doc_type: str | None = None, top_k: int = 5
) -> list[dict[str, Any]]:
    """
    Execute Hybrid Search for legal chunks.
    """
    logger.info(
        f"[Tool:search_legal_clauses] Query: '{query}', Target Date: {target_date}, Top K: {top_k}"
    )
    chunks = hybrid_retriever.search(
        query=query, target_date=target_date, top_k=top_k, doc_type=doc_type
    )
    # Fallback retry without temporal filter if no results and target_date was set
    if not chunks and target_date:
        logger.info(
            "[Tool:search_legal_clauses] Fallback: Retrying search without target_date filter..."
        )
        chunks = hybrid_retriever.search(
            query=query, target_date=None, top_k=top_k, doc_type=doc_type
        )
    return chunks


def get_law_document_detail(document_id: str) -> dict[str, Any] | None:
    """
...
def get_law_article(document_id: str, article_number: int) -> dict[str, Any] | None:
    """
