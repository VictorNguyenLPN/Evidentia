...
from backend.config import DATA_PATH, FREE_PLAN_QUESTION_LIMIT, GEMINI_MODEL, PLAN_QUESTION_LIMITS
...
def get_system_health(
    ...
    health["services"]["qdrant"] = {
        ...
        "status": "healthy"
        if qdrant_healthy
        else ("unconfigured" if not qdrant_configured else "error"),
        "latency_ms": t_qdrant,
        "collection": qdrant_manager.collection_name,
    ...
    health["services"]["embeddings"] = {
        ...
        "dense_model": getattr(
            embedding_service, "dense_model_name", "bkai-foundation-models/vietnamese-bi-encoder"
        ),
        "status": "healthy",
    }
...
def reset_user_password(
    ...
    if not ok:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Không thể cập nhật mật khẩu."
        )
...
def reset_all_user_limits(
    ...
    plan_desc = (
        f"thuộc gói '{payload.plan}'" if payload.plan and payload.plan != "all" else "toàn hệ thống"
    )
...
def reset_guest_limits(
    ...
    if payload.ip:
        mongo_manager.reset_user_question_count_by_identifier(payload.ip)
