...
class QdrantCloudManager:
    ...
    def create_collection(self) -> None:
        ...
        logger.info(f"Creating Qdrant Cloud collection '{self.collection_name}' with Hybrid Vectors (Dense dim={dense_dim})...")
    ...
    def ingest_chunks(self, chunks: List[Dict[str, Any]], batch_size: int = 32) -> int:
        ...
        client = self.get_client()
        total = len(chunks)
        logger.info(f"Starting ingestion of {total} chunks to Qdrant Cloud collection '{self.collection_name}'...")
        ...
        for i in range(0, total, batch_size):
            ...
            logger.info(f"Uploaded batch {i // batch_size + 1}/{(total + batch_size - 1) // batch_size} ({len(points)} chunks).")
    ...
    def sync_data_on_startup(self, data_path: Path = DATA_PATH) -> Dict[str, Any]:
        ...
        logger.info(f"Checking Qdrant Cloud synchronization with local file (count = {local_count})")
